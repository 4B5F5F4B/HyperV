					     ...
					     u32 msg_len)
{
	...
	switch(msg_type)
	{
		case RNDIS_MSG_QUERY:
			printk("[*]hv_netvsc send rndis packet:RNDIS_MSG_QUERY\n");
			break;

		case RNDIS_MSG_SET:
			printk("[*]hv_netvsc send rndis packet:RNDIS_MSG_SET\n");
			break;

		case RNDIS_MSG_INIT:
			printk("[*]hv_netvsc send rndis packet:RNDIS_MSG_INIT\n");
			break;

		case RNDIS_MSG_HALT:
			printk("[*]hv_netvsc send rndis packet:RNDIS_MSG_HALT\n");
			break;
	}
...
int rndis_filter_send_request(struct rndis_device *dev,
				  struct rndis_request *req)
{
...
int rndis_filter_query_device_mac(struct rndis_device *dev,
				    ...
				    struct netvsc_device *nvdev)
{
	...
	/*
		in order to send NVSP_MSG1_TYPE_SEND_RECV_BUF we need to skip send RNDIS_MSG_INIT
		message
	*/
	...
	if (status == RNDIS_STATUS_SUCCESS) {
		...
		nvdev->max_pkt = RNDIS_MAX_PKT_DEFAULT;
		nvdev->pkt_align = RNDIS_PKT_ALIGN_DEFAULT;
		//printk("[*]hv_netvsc:max_pkt:%d pkt_align:%d\n", nvdev->max_pkt, nvdev->pkt_align);
		ret = 0;
	} else {
		dev->state = RNDIS_DEV_UNINITIALIZED;
		ret = -EINVAL;
...
static void netvsc_sc_open(struct vmbus_channel *new_sc)
{
	...
	printk("[*]hv_netvsc netvsc_sc_open channel:0x%010x\n", new_sc);
...
void rndis_set_subchannel(struct work_struct *w)
{
	...
	init_packet->msg.v5_msg.subchn_req.num_subchannels =
						nvdev->num_chn - 1;
	printk("[*]hv_netvsc: vmbus_sendpacket NVSP_MSG5_TYPE_SUBCHANNEL\n");
