...
extern bool g_block_rndis_thread;
...
void netvsc_switch_datapath(struct net_device *ndev, bool vf)
{
	...
	printk("[*]hv_netvsc: vmbus_sendpacket NVSP_MSG4_TYPE_SWITCH_DATA_PATH\n");
...
static void netvsc_destroy_buf(struct hv_device *device)
{
	...
	if (net_device->recv_section_cnt) {
		...
		printk("[*]hv_netvsc: vmbus_sendpacket NVSP_MSG1_TYPE_REVOKE_RECV_BUF\n");
	...
	if (net_device->send_section_cnt) {
		...
		printk("[*]hv_netvsc: vmbus_sendpacket NVSP_MSG1_TYPE_REVOKE_SEND_BUF\n");
		ret = vmbus_sendpacket(device->channel,
				       revoke_packet,
				       sizeof(struct nvsp_message),
				       (unsigned long)revoke_packet,
			   ...
			   const struct netvsc_device_info *device_info)
{
	...
	printk("[*]hv_netvsc: vmbus_sendpacket NVSP_MSG1_TYPE_SEND_RECV_BUF\n");
	...
	printk("[*]hv_netvsc: vmbus_sendpacket NVSP_MSG1_TYPE_SEND_SEND_BUF\n");
	/* Send the gpadl notification request */
	ret = vmbus_sendpacket(device->channel, init_packet,
			       sizeof(struct nvsp_message),
			       (unsigned long)init_packet,
			      ...
			      u32 nvsp_ver)
{
	...
	printk("[*]hv_netvsc: vmbus_sendpacket NVSP_MSG_TYPE_INIT\n");
	...
	printk("[*]hv_netvsc: vmbus_sendpacket NVSP_MSG2_TYPE_SEND_NDIS_CONFIG\n");
	ret = vmbus_sendpacket(device->channel, init_packet,
				sizeof(struct nvsp_message),
				(unsigned long)init_packet,
				VM_PKT_DATA_INBAND, 0);
			      ...
			      const struct netvsc_device_info *device_info)
{
	...
	init_packet->msg.v1_msg.
		send_ndis_ver.ndis_minor_ver =
				ndis_version & 0xFFFF;
	printk("[*]hv_netvsc: vmbus_sendpacket NVSP_MSG1_TYPE_SEND_NDIS_VER\n");
	/* Send the init request */
	ret = vmbus_sendpacket(device->channel, init_packet,
				sizeof(struct nvsp_message),
				(unsigned long)init_packet,
	...
	struct sk_buff *skb)
{
	...
	printk("[*]hv_netvsc netvsc_send_pkt channel:0x%016x idx:%d\n", out_channel, packet->q_idx);
	...
	if (packet->page_buf_cnt) {
		...
		printk("[*]hv_netvsc: vmbus_sendpacket NVSP_MSG1_TYPE_SEND_RNDIS_PKT\n");
...
int send_recv_completions(struct net_device *ndev,
				 struct netvsc_device *nvdev,
				 struct netvsc_channel *nvchan)
{
	...
	while (mrc->first != mrc->next) {
		...
		printk("[*]hv_netvsc send NVSP_MSG1_TYPE_SEND_RNDIS_PKT_COMPLETE(channel:0x%016x).\n", nvchan->channel);
