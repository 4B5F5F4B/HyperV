...
int main()
{
	...
	/*printf("[*]hyper_v: send first rndis query.\n");
	...
	printf("[*]hyper_v: block rndis thread.\n");
	block = 1;
	ret = ioctl(fd, HYPERV_IOCTL_SET_BLOCK_RNDIS_THREAD, &block, sizeof(block));
	if(0	!= ret)
	{
		perror("[*]hyper_v: block rndis thread fail.");
		goto __EXIT;
	}

	printf("[*]hyper_v: send second rndis query.\n");
	ret = ioctl(fd, HYPERV_IOCTL_QUERY, &recv_buf_size, sizeof(recv_buf_size));
	if(0	!=	ret)
	{
		perror("[*]hyper_v send rndis query fail.");
	...
	for(int i=0; i<10; i++)
	{
		printf("[*]hyper_v: send %d rndis keep alive.\n", i);
		ret = ioctl(fd, HYPERV_IOCTL_SEND_KEEPALIVE, &recv_buf_size, sizeof(recv_buf_size));
		if(0	!=	ret)
		{
			perror("[*]hyper_v send rndis keep alive.");
			goto __EXIT;	
		}
	}
